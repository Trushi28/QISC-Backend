...
int load_operand(qisc_bytebuf* buf, qisc_value* val, qisc_live_interval** id_to_inter, int default_reg) {
    if (val->kind == QISC_VAL_CONST_INT) {
        int64_t v = val->as.i_val;
        ...
        if (val->as.param_idx < 6) {
            int stack_slot = 16 + val->as.param_idx * 8;
            emit_rex(buf, 1, default_reg >= 8 ? 1 : 0, 0, 0);
            bytebuf_push(buf, 0x8B);
            emit_modrm(buf, 2, default_reg & 7, 5);
            bytebuf_push_u32(buf, (uint32_t)(-stack_slot));
            return default_reg;
        }
    }
    return default_reg;
...
bool qisc_codegen_emit_elf(qisc_ir_module* mod, const char* filepath) {
    ...
    for (qisc_ir_function* f = mod->first_func; f; f = f->next) {
        ...
        for (int i=0; i<num_intervals; i++) {
            ...
            for (int j=0; j<num_active; j++) {
                int idx = active[j];                if (intervals[idx].end < start) {
        ...
        int p_regs[] = {7, 6, 2, 1, 8, 9};
        for(int p=0; p<6; p++) {
            int stack_slot = 16 + p * 8;
            int r = p_regs[p];
            emit_rex(buf, 1, r >= 8 ? 1 : 0, 0, 0);
            bytebuf_push(buf, 0x89);
            emit_modrm(buf, 2, r & 7, 5);
            bytebuf_push_u32(buf, (uint32_t)(-stack_slot));
        }
        ...
        for(qisc_ir_block* b = f->first_block; b; b = b->next) {
            ...
            for(qisc_ir_inst* i = b->first_inst; i; i = i->next) {
                ...
                if (i->opcode == QISC_OP_NOP) {
                    ...
                    int op0 = load_operand(buf, i->operands[0], id_to_inter, 0);
                    ...
                    int op0 = load_operand(buf, i->operands[0], id_to_inter, 11);
                    int op1 = load_operand(buf, i->operands[1], id_to_inter, 15);
                    ...
                    int src = load_operand(buf, i->operands[0], id_to_inter, 11);
                    ...
                    int dst_addr = load_operand(buf, i->operands[0], id_to_inter, 11);
                    int src_val = load_operand(buf, i->operands[1], id_to_inter, 15);
                    ...
                    for (size_t arg=1; arg<i->num_operands; arg++) {
                        int arg_reg = load_operand(buf, i->operands[arg], id_to_inter, 11);
                    ...
                    int cond = load_operand(buf, i->operands[0], id_to_inter, 11);
                    ...
                    int op2 = load_operand(buf, i->operands[2], id_to_inter, 11);
    ...
    size_t symtab_offset = cold_offset + cold_code_size;
    size_t symtab_size = (1 + num_func_codes + 7) * sizeof(Elf64_Sym);
    ...
    const char shstrtab[64] = "\0.text.hot\0.text.cold\0.symtab\0.strtab\0.shstrtab\0.rela.text\0";
    size_t shstrtab_size = 64;
    size_t rela_offset = shstrtab_offset + shstrtab_size;
    size_t rela_size = num_relocs * sizeof(Elf64_Rela);
    ...
    ehdr.e_shnum = 7; ehdr.e_shstrndx = 5;
    ...
    Elf64_Sym ext_syms[7]; memset(ext_syms, 0, sizeof(ext_syms));
    for (int i=0; i<7; i++) {
        ext_syms[i].st_name = name_off; ext_syms[i].st_info = 0x10;
        if (i==0) name_off += strlen("__qisc_try_begin") + 1;
        if (i==1) name_off += strlen("__qisc_try_catch") + 1;
        if (i==2) name_off += strlen("__qisc_fail") + 1;
        if (i==3) name_off += strlen("__qisc_stream_map") + 1;
        if (i==4) name_off += strlen("__qisc_stream_filter") + 1;
        if (i==5) name_off += strlen("__qisc_stream_reduce") + 1;
        if (i==6) name_off += strlen("__qisc_pipeline") + 1;
    ...
    for (int i=0; i<num_relocs; i++) {
        ...
        int sym_idx = 0;
        if (strcmp("__qisc_try_begin", relocs[i].symbol_name) == 0) sym_idx = 1;
        else if (strcmp("__qisc_try_catch", relocs[i].symbol_name) == 0) sym_idx = 2;
        else if (strcmp("__qisc_fail", relocs[i].symbol_name) == 0) sym_idx = 3;
        else if (strcmp("__qisc_stream_map", relocs[i].symbol_name) == 0) sym_idx = 4;
        else if (strcmp("__qisc_stream_filter", relocs[i].symbol_name) == 0) sym_idx = 5;
        else if (strcmp("__qisc_stream_reduce", relocs[i].symbol_name) == 0) sym_idx = 6;
        else if (strcmp("__qisc_pipeline", relocs[i].symbol_name) == 0) sym_idx = 7;
        else {
            for (int j=0; j<num_func_codes; j++) {
                if (strcmp(func_codes[j].func->name, relocs[i].symbol_name) == 0) { sym_idx = j + 8; break; }
            }
        }
        size_t func_base = 0;
        for (int j=0; j<num_func_codes; j++) {
        ...
        r.r_info = ELF64_R_INFO(sym_idx, 4);
        r.r_addend = -4;
    ...
    Elf64_Shdr shdrs[7]; memset(shdrs, 0, sizeof(shdrs));
    ...
    shdrs[3].sh_name = 22; shdrs[3].sh_type = 2; shdrs[3].sh_offset = symtab_offset; shdrs[3].sh_size = symtab_size; shdrs[3].sh_link = 4; shdrs[3].sh_info = 8; shdrs[3].sh_entsize = 24;
    shdrs[4].sh_name = 30; shdrs[4].sh_type = 3; shdrs[4].sh_offset = strtab_offset; shdrs[4].sh_size = strtab_size;
    shdrs[5].sh_name = 38; shdrs[5].sh_type = 3; shdrs[5].sh_offset = shstrtab_offset; shdrs[5].sh_size = shstrtab_size;
    shdrs[6].sh_name = 48; shdrs[6].sh_type = 4; shdrs[6].sh_offset = rela_offset; shdrs[6].sh_size = rela_size; shdrs[6].sh_link = 3; shdrs[6].sh_info = 1; shdrs[6].sh_entsize = 24;
    
    fwrite(shdrs, sizeof(Elf64_Shdr), 7, f);
    fclose(f);
